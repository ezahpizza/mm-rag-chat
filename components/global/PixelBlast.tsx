...
export const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      renderer.domElement.style.pointerEvents = 'none'; // Disable pointer events on canvas
      ...
      const mapToPixels = (e: PointerEvent) => {
        const rect = container.getBoundingClientRect(); // Use container instead of canvas
      ...
      // Add listeners to container to capture interactions for ripple effects
      // Canvas has pointer-events: none, so UI interactions pass through
      ...
      if (t.liquidEffect) {
        const uStrength = (t.liquidEffect as Effect).uniforms.get('uStrength');
        if (uStrength) uStrength.value = liquidStrength;
        const uFreq = (t.liquidEffect as Effect).uniforms.get('uFreq');
