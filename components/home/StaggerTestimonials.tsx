    ...
    >
      ...
      <div className="absolute bottom-4 left-1/2 flex -translate-x-1/2 gap-8">
        <button
          ...
          className="grid h-14 w-14 place-content-center text-3xl bg-electric transition-colors hover:bg-persian hover:text-white"
        >
          <FiChevronLeft />
        </button>
        <button
          onClick={() => handleMove(1)}
          className="grid h-14 w-14 place-content-center text-3xl bg-electric transition-colors hover:bg-persian hover:text-pearl"
        >
          <FiChevronRight />
        </button>
      </div>
  ...
  return (
    <motion.div
      ...
      absolute left-1/2 top-1/2 cursor-pointer border-black p-8 text-black transition-colors duration-500 ${
        isActive ? "z-10 bg-razza" : "z-0 bg-persian"
      }
      `}
      style={{
    ...
    >
      ...
      <h3
        className={`text-base sm:text-xl ${
          isActive ? "text-black" : "text-pearl"
        }`}
      >
      ...
      <p
        className={`absolute bottom-8 left-8 right-8 mt-2 text-sm italic ${
          isActive ? "text-indigo-200" : "text-skye/40"
        }`}
      >
        - {testimonial.by}
      </p>
